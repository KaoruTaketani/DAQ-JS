...
socket.addEventListener('message', event => {
    const key = 'channelEventOffsetValue'
...
offsetElement.onchange = () => {
    ...
    socket.send(JSON.stringify({ channelEventOffset: value }))
}
...
socket.addEventListener('message', event => {
    const key = 'channelEventMessageInnerText'
    if (!event.data.startsWith(`{"${key}":"`)) return
...
socket.addEventListener('message', event => {
    const key = 'channelEventTableInnerHTML'
    if (!event.data.startsWith(`{"${key}":`)) return
