...
export default (
    ...
    let polyline
    if (axes.yScale === 'log' && axes.xScale === 'log') {
        polyline = [
            `<polyline`,
    ...
    const xTexts = axes.xTick.map((x, i) => {
        const r = axes.xScale === 'log'
            ? (Math.log10(x) - Math.log10(xMinInData)) / (Math.log10(xMaxInData) - Math.log10(xMinInData))
            : (x - xMinInData) / (xMaxInData - xMinInData)
        const xTickInPixels = xMinInPixels + r * (xMaxInPixels - xMinInPixels)
        return [`<text`,
        ...
        const r = axes.yScale === 'log'
            ? (Math.log10(y) - Math.log10(yMinInData)) / (Math.log10(yMaxInData) - Math.log10(yMinInData))
            : (y - yMinInData) / (yMaxInData - yMinInData)
            const yTickInPixels = yMinInPixels
                + r * (yMaxInPixels - yMinInPixels)
