...
test('example inputs in lsqcurvefit of matlab', () => {
    const xdata = [0.9, 1.5, 13.8, 19.8, 24.1, 28.2, 35.2, 60.3, 74.6, 81.],
        ...
        fun = [
            (/** @type {number[]} */p) => xdata.map((_, i) => ydata[i] - p[0] * Math.exp(p[1] * xdata[i])),
            (/** @type {number[]} */p) => xdata.map((_, i) => [
    ...
    isapprox(p[0], 498.83094)
    isapprox(p[1], -0.101257)
})

test('example inputs in lsqnonlin of matlab', () => {
    // x = linspace(0, 3),
    // y = x.map(d => Math.exp(-1.3 * d) + 0.05 * (Math.random() - 0.5)),
    // replaced randn by rand in the example input
    /** @type {import('./index.js').XY} */
    const xy = JSON.parse(readFileSync('./lsqnonlin.json', 'utf8')),
        fun = [
            (/** @type {number[]} */r) => xy.x.map((_, i) => Math.exp(-xy.x[i] * r[0]) - xy.y[i]),
            (/** @type {number[]} */r) => xy.x.map((_, i) => [-xy.x[i] * Math.exp(-xy.x[i] * r[0])])
        ],
