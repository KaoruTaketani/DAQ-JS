...
export default (
    ...
    const points = y.binCounts.map((_, i) => {
        if (axes.yScale === 'log') {
            return [
                `${(xMinInPixels + (xMaxInPixels - xMinInPixels) * (i * binWidth - xMin) / (xMax - xMin)).toFixed()},${(yMinInPixels + (yMaxInPixels - yMinInPixels) * (Math.log10(y.binCounts[i]) - Math.log10(yMin)) / (Math.log10(yMax) - Math.log10(yMin))).toFixed()}`,
                `${(xMinInPixels + (xMaxInPixels - xMinInPixels) * ((i + 1) * binWidth - xMin) / (xMax - xMin)).toFixed()},${(yMinInPixels + (yMaxInPixels - yMinInPixels) * (Math.log10(y.binCounts[i]) - Math.log10(yMin)) / (Math.log10(yMax) - Math.log10(yMin))).toFixed()}`
            ].join(' ')
            // return [
            ...
            return [
                `${(xMinInPixels + (xMaxInPixels - xMinInPixels) * (i * binWidth - xMin) / (xMax - xMin)).toFixed()},${(yMinInPixels + (yMaxInPixels - yMinInPixels) * (y.binCounts[i] - yMin) / (yMax - yMin)).toFixed()}`,
                `${(xMinInPixels + (xMaxInPixels - xMinInPixels) * ((i + 1) * binWidth - xMin) / (xMax - xMin)).toFixed()},${(yMinInPixels + (yMaxInPixels - yMinInPixels) * (y.binCounts[i] - yMin) / (yMax - yMin)).toFixed()}`
            ].join(' ')
            // return [
