...
export default (
    ...
    //     // console.log(`${i},${y.binLimits[0]} ${(i + 1) * binWidth} <= ${xMin} ${y.binLimits[0] + (i + 1) * binWidth <= xMin}, ${i * binWidth} >= ${xMax} ${y.binLimits[0] + i * binWidth >= xMax}`)
    //     if (y.binLimits[0] + (i + 1) * binWidth <= xMin || y.binLimits[0] + i * binWidth >= xMax) {
    //         // console.log(i)
    //         return ''
    //     }
    //     if (axes.yScale === 'log') {
    //         const ry = y.binCounts[i] <= 0 ? 0 : (Math.log10(y.binCounts[i]) - Math.log10(yMin)) / (Math.log10(yMax) - Math.log10(yMin)),
    //             rx0 = (i * binWidth + y.binLimits[0] - xMin) / (xMax - xMin),
    //             rx1 = ((i + 1) * binWidth + y.binLimits[0] - xMin) / (xMax - xMin)
    ...
    return `<polyline points="${points.trim()}" stroke="black" fill="none" />`
}
