...
export default (
    ...
    if (axes.xScale === 'log' && xMin <= 0) {
        const xInPixcels = 0.5 * (xMinInPixels + xMaxInPixels)
        const yInPixels = 0.5 * (yMinInPixels + yMaxInPixels)
        return `<text x="${xInPixcels.toFixed()}" y="${yInPixels.toFixed()}" text-anchor="middle" font-size="30" fill="red">x min must be positive</text>`
    }
    const rx = axes.xScale === 'log'
        ? Array.from(x).map((_, i) => (Math.log10(x[i]) - Math.log10(xMin)) / (Math.log10(xMax) - Math.log10(xMin)))
        : Array.from(x).map((_, i) => (x[i] - xMin) / (xMax - xMin))

    const ry = axes.yScale === 'log'
        ? Array.from(y).map((_, i) => (Math.log10(y[i]) - Math.log10(yMin)) / (Math.log10(yMax) - Math.log10(yMin)))
        : Array.from(y).map((_, i) => (y[i] - yMin) / (yMax - yMin))


    return Array.from(x).map((_, i) => {
        if (Number.isNaN(rx[i])) return ''
        if (Number.isNaN(ry[i])) return ''

        const xInPixels = xMinInPixels + rx[i] * (xMaxInPixels - xMinInPixels)
        const yInPixels = yMinInPixels + ry[i] * (yMaxInPixels - yMinInPixels)
